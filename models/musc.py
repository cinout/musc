    ...
    def make_category_data(self, category):
        ...
        if self.online:
            ...
            for batch_idx, image_info in enumerate(test_dataloader):
                ...
                anomaly_maps_iter = anomaly_maps_iter.cpu().numpy()
                torch.cuda.empty_cache()

                # TODO: does online mode require RsCIN???
                scores_cls_iter = (
            ...
            class_tokens = (
                []
            )  # TODO: something to think about when using RsCIN for online mode
            ...
            for divide_iter in range(divide_num):
                ...
                # extract features
                patch_tokens_list = []  #  accumulates from batch to batch
            ...
            # start of image-level score calculation
            B = anomaly_maps.shape[0]  # the number of unlabeled test images
            ac_score = (
                np.array(anomaly_maps).reshape(B, -1).max(-1)
            )  # raw pixel-level AC score, shape: [#test, ]
            # RsCIN
            if self.dataset == "visa":
                k_score = [1, 8, 9]
